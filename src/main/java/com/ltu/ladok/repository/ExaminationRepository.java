...
public interface ExaminationRepository extends JpaRepository<Examination, Long> {
    ...
    @Query(value = "SELECT * FROM examination WHERE course_instance_fk = :courseInstanceId",
            nativeQuery = true)
    List<Examination> findByCourseInstanceId(@Param("courseInstanceId") Long courseInstanceId);

    @Query(value = "SELECT * FROM examination WHERE course_instance_fk = :courseInstanceId AND provnummer = :provNummer",
            nativeQuery = true)
